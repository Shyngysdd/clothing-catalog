...
function HomeProductPreview({ product, tone }: { product: CatalogProduct; tone: string }) {
  ...
  return <div className="look-product-media aspect-[4/5]" style={{ "--look-tone": tone, backgroundColor: product.imageColor } as CSSProperties} onMouseEnter={() => setActiveFrame(frames.length > 1 ? 1 : 0)} onMouseLeave={() => setActiveFrame(0)}>
    <div className={`look-product-photo-layer ${product.sizes.length > 0 && product.sizes.every((size) => !size.inStock) ? "grayscale" : ""}`}>{frames.map((imageUrl, index) => <div key={imageUrl} className={`look-gallery-frame ${activeFrame === index ? "is-active" : ""}`}><Image src={imageUrl} alt="" aria-hidden="true" fill sizes="(max-width: 767px) 72vw, 18rem" quality={25} className="product-card-photo-blur-bg" /><Image src={imageUrl} alt="" fill sizes="(max-width: 767px) 72vw, 18rem" className="product-card-photo-sharp" /></div>)}</div>
