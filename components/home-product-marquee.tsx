...
export function HomeProductMarquee({ products: selection, manual = false }: { products: CatalogProduct[]; manual?: boolean }) {
  ...
  return <div className={`home-product-marquee relative ${manual ? "home-product-marquee--manual" : ""}`} onTouchStart={pauseForTouch} onTouchEnd={resumeAfterTouch} onTouchCancel={resumeAfterTouch}>
    ...
    <div ref={trackRef} className={`home-product-track ${shouldLoop ? "" : "home-product-track--static"}`} style={{ animationPlayState: isUserScrolling ? "paused" : "running" }}>{loopedProducts.map((product, index) => <Link key={`${product.id}-${index}`} href={`/catalog/${product.id}`} className="home-look-card look-product-card group flex h-full flex-col" aria-label={`${catalog("title")}: ${getLocalizedField(product, "name", locale)}`}><p className="product-card-kicker font-mono-price mb-3 text-[0.62rem] tracking-[0.13em] text-[color:var(--accent)]">{catalog("sku", {sku: product.sku})}</p><HomeProductPreview product={product} tone={index % 2 === 0 ? "var(--accent)" : "var(--gold)"} /><div className="mt-4 flex min-h-[5.75rem] flex-1 flex-col md:min-h-0 md:flex-row md:items-start md:justify-between md:gap-3"><div><p className="font-mono-price text-[0.6rem] tracking-[0.12em] text-[color:var(--accent)]">{product.brand.toUpperCase()}</p><h3 className="mt-1 line-clamp-2 min-h-[2.5rem] text-sm font-medium leading-5 sm:text-base md:min-h-0">{getLocalizedField(product, "name", locale)}</h3></div><div className="mt-auto min-h-[2.25rem] pt-2 text-left md:mt-0 md:min-h-0 md:shrink-0 md:pt-0 md:text-right"><p className="font-mono-price text-sm">{formatPrice.format(product.price)} ₸</p>{product.originalPrice ? <p className="font-mono-price mt-0.5 text-[0.65rem] text-[color:var(--ink)]/45 line-through">{formatPrice.format(product.originalPrice)} ₸</p> : null}</div></div></Link>)}</div>
  </div>;
}
