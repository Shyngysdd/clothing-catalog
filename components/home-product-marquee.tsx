...
export function HomeProductMarquee({ products: selection }: { products: CatalogProduct[] }) {
  ...
  return <div className="home-product-marquee" onTouchStart={pauseForTouch} onTouchEnd={resumeAfterTouch} onTouchCancel={resumeAfterTouch}><div className={`home-product-track ${shouldLoop ? "" : "home-product-track--static"}`} style={{ animationPlayState: isUserScrolling ? "paused" : "running" }}>{loopedProducts.map((product, index) => <Link key={`${product.id}-${index}`} href={`/catalog/${product.id}`} className="home-look-card look-product-card group flex h-full flex-col" aria-label={`Открыть товар: ${product.name}`}><p className="look-number mb-3 text-3xl text-[color:var(--ink)] sm:text-4xl">LOOK {String((index % selection.length) + 1).padStart(2, "0")}</p><HomeProductPreview product={product} tone={index % 2 === 0 ? "var(--accent)" : "var(--gold)"} /><div className="mt-4 flex min-h-[4.75rem] flex-1 flex-col md:min-h-0 md:flex-row md:items-start md:justify-between md:gap-3"><h3 className="line-clamp-2 min-h-[2.5rem] text-sm font-medium leading-5 sm:text-base md:min-h-0">{product.name}</h3><div className="mt-auto min-h-[2.25rem] pt-2 text-left md:mt-0 md:min-h-0 md:shrink-0 md:pt-0 md:text-right"><p className="font-mono-price text-sm">{formatPrice.format(product.price)} ₸</p>{product.originalPrice ? <p className="font-mono-price mt-0.5 text-[0.65rem] text-[color:var(--ink)]/45 line-through">{formatPrice.format(product.originalPrice)} ₸</p> : null}</div></div></Link>)}</div></div>;
}
