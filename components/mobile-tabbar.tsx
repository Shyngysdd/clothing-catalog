...
export function MobileTabbar() {
  ...
  return <nav aria-label="Мобильная навигация" className="fixed inset-x-0 bottom-0 z-50 flex h-[4.5rem] border-t border-[color:var(--ink)]/20 bg-[color:var(--paper)] px-1 pb-[env(safe-area-inset-bottom)] sm:hidden">{tabs.map((tab) => { const isActive = tab.matches(pathname); const badgeCount = tab.href === "/cart" ? itemCount : tab.href === "/favorites" ? favoriteIds.length : 0; return <Link key={tab.href} href={tab.href} className={`relative flex min-w-0 flex-1 flex-col items-center justify-center gap-1 font-mono-price text-[0.55rem] tracking-[0.02em] ${isActive ? "text-[color:var(--accent)]" : "text-[color:var(--ink)]/55"}`} aria-current={isActive ? "page" : undefined}><span className="relative">{tab.icon}{badgeCount > 0 ? <span className="absolute -right-3 -top-2 grid min-w-4 place-items-center border border-[color:var(--paper)] bg-[color:var(--accent)] px-1 text-[0.55rem] leading-4 text-[color:var(--white)]">{badgeCount}</span> : null}</span><span>{tab.label}</span></Link>; })}</nav>;
}
