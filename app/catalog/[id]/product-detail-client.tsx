        ...
        <section>
          <div className="look-preview aspect-[4/5]" style={{ backgroundColor: product.imageColor }}>
            {galleryImages.length > 0
              ? galleryImages.map((imageUrl, index) => <div key={imageUrl} className={`look-gallery-frame ${activeFrame === index ? "is-active" : ""}`}><Image src={imageUrl} alt="" aria-hidden="true" fill sizes="(max-width: 1024px) 100vw, 50vw" quality={25} className="product-detail-photo-blur-bg" /><Image src={imageUrl} alt={`${product.name}, кадр ${index + 1}`} fill sizes="(max-width: 1024px) 100vw, 50vw" className="product-detail-photo-sharp" /></div>)
