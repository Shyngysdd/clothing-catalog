...
export default async function CustomerLoginPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return (
    <main className="mx-auto grid min-h-[calc(100svh-73px)] max-w-md place-items-center px-4 py-12">
      <form action={loginCustomer} className="w-full border border-[color:var(--ink)]/20 bg-[color:var(--white)] p-6 sm:p-8">
        ...
        {error ? <p className="mt-3 text-sm text-[color:var(--accent)]">Неверный email или пароль.</p> : null}
