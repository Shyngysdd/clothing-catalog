...
function CatalogProductPreview({ product, index, isSoldOut }: { product: CatalogProduct; index: number; isSoldOut: boolean }) {
  ...
  return (
    ...
    >
      ...
      {isSoldOut ? <span className="absolute right-3 top-3 border border-[color:var(--paper)]/50 bg-[color:var(--ink)]/80 px-2 py-1 font-mono-price text-[0.65rem] text-[color:var(--white)]">{t("soldOut")}</span> : null}
    ...
    <section className="catalog-page px-4 py-10 sm:px-6 sm:py-16 lg:px-10">
      <div className="catalog-page-header mb-10 border-b border-[color:var(--border)] pb-8 sm:mb-14 sm:pb-10">
        <div>
          ...
          <h1 className="font-display mt-3 text-[clamp(3.2rem,9vw,6.5rem)] leading-[0.8]">{department === "men" ? header("men") : department === "women" ? header("women") : t("title")}</h1>
        ...
        <button
          type="button"
          className="catalog-filter-trigger inline-flex min-h-11 items-center border border-[color:var(--border)] px-4 text-sm font-medium hover:border-[color:var(--accent)] hover:text-[color:var(--accent)] md:hidden"
      ...
      <div className="catalog-page-layout flex flex-col gap-8 md:flex-row">
        <aside
          id="catalog-filters"
          className={`${filtersOpen ? "block" : "hidden"} catalog-filter-panel shrink-0 border-b border-[color:var(--border)] pb-8 md:block md:w-60 md:border-b-0 md:border-r md:pb-0 md:pr-8`}
        >
          ...
          <label className="mt-6 block">
            ...
            <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти товар..." className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--paper)] px-3 text-sm outline-none placeholder:text-[color:var(--ink)]/45 focus:border-[color:var(--accent)]" />
          </label>

          <div className="mt-6">
            <p className="text-sm font-medium">Сортировка</p>
            <div className="catalog-sort-control relative mt-2">
              <button type="button" onClick={() => setIsSortOpen((isOpen) => !isOpen)} aria-haspopup="listbox" aria-expanded={isSortOpen} className="flex min-h-11 w-full items-center justify-between border border-[color:var(--border)] bg-transparent px-3 text-left text-sm transition-colors hover:border-[color:var(--accent)] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--accent)]"><span>{sortOptions.find((option) => option.value === sort)?.label}</span><svg aria-hidden="true" viewBox="0 0 16 16" className={`size-4 transition-transform ${isSortOpen ? "rotate-180" : ""}`} fill="none" stroke="currentColor" strokeWidth="1.6"><path d="m3 5.5 5 5 5-5" /></svg></button>
          ...
          <fieldset className="mt-6">
            ...
            <div className="mt-3 space-y-3">
              {[null, ...categories].map((item) => {
                ...
                return (
                <label key={slug} className="flex cursor-pointer items-center gap-3 text-sm text-[color:var(--ink)]/75">
                  <input
                    type="radio"
                    name="category"
                    value={slug}
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <label className="min-w-0">
                  ...
                  <input
                    ...
                    className="mt-1 min-h-10 w-full border border-[color:var(--border)] bg-transparent px-2 text-sm tabular-nums outline-none focus:border-[color:var(--accent)]"
                    aria-label="Цена от"
                  />
                </label>
                <label className="min-w-0">
        ...
        <div className="catalog-results min-w-0 flex-1">
          {debouncedSearch ? <div className="mb-5 flex flex-wrap items-center justify-between gap-3 border-b border-[color:var(--border)] pb-4 text-sm"><p>Результаты по запросу «{debouncedSearch}»: {filteredProducts.length} {filteredProducts.length === 1 ? "товар" : "товаров"}</p><button type="button" onClick={() => { setSearch(""); setDebouncedSearch(""); }} className="text-sm text-[color:var(--accent)] underline underline-offset-4">Сбросить поиск</button></div> : null}
          <div className="mb-5 flex items-center justify-end gap-1 md:hidden" aria-label="Плотность сетки">
            <span className="mr-2 font-mono-price text-[0.65rem] tracking-[0.08em] text-[color:var(--ink)]/55">{t("grid")}</span>
            <button
              type="button"
              onClick={() => setGridDensity("1")}
              aria-label={t("oneColumn")}
              aria-pressed={gridDensity === "1"}
              className={`grid size-9 place-items-center border transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--accent)] ${gridDensity === "1" ? "border-[color:var(--ink)] bg-[color:var(--ink)] text-[color:var(--white)]" : "border-[color:var(--border)] text-[color:var(--ink)]/60"}`}
            >
            ...
            <button
              ...
              className={`grid size-9 place-items-center border transition-colors focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[color:var(--accent)] ${gridDensity === "2" ? "border-[color:var(--ink)] bg-[color:var(--ink)] text-[color:var(--white)]" : "border-[color:var(--border)] text-[color:var(--ink)]/60"}`}
            >
          ...
          {sortedProducts.length > 0 ? (
            <div data-density={gridDensity === "2" ? "compact" : "comfortable"} className={`catalog-product-grid grid items-stretch ${gridDensity === "2" ? "grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4" : "grid-cols-1"} gap-x-6 gap-y-8`}>
            ...
            <div className="border border-dashed border-[color:var(--border)] px-6 py-12 text-center">
