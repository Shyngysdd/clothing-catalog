...
export default async function AccountPage({ params, searchParams }: { params: Promise<{ locale: string }>; searchParams: Promise<{ verification?: string; profile?: string; password?: string }> }) {
  ...
  const allProducts = await prisma.product.findMany({ include: { sizes: true, category: true }, orderBy: { createdAt: "desc" } });

  return (
    <main className="account-page mx-auto max-w-[100rem] px-4 py-10 sm:px-6 sm:py-16">
      <header className="account-page-header border-b border-[color:var(--border)] pb-8 sm:pb-10"><div><p className="font-mono-price text-xs tracking-[0.16em] text-[color:var(--accent)]">ЛИЧНЫЙ КАБИНЕТ</p><h1 className="font-display mt-3 text-[clamp(3.2rem,8vw,6.2rem)] leading-[0.8]">Здравствуйте,<br />{customer.name}</h1></div><p>Здесь хранятся ваши данные, заказы, избранные вещи и адреса для следующего оформления.</p></header>
      <div className="account-settings-grid mt-10">
      <section className="account-data-section border-y border-[color:var(--border)] py-6">
        ...
        <form action={updateProfile} className="mt-6 grid gap-5 sm:grid-cols-2">
          <label className="block text-sm font-medium">Имя<input name="name" defaultValue={customer.name} autoComplete="name" required className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--white)] px-3 text-sm font-normal outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block text-sm font-medium">Телефон<input name="phone" type="tel" defaultValue={customer.phone ?? ""} autoComplete="tel" className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--white)] px-3 text-sm font-normal outline-none focus:border-[color:var(--accent)]" /></label>
      ...
      <section className="account-data-section border-y border-[color:var(--border)] py-6">
        ...
        <form action={changePassword} className="mt-6 grid gap-5 sm:grid-cols-2">
          <label className="block text-sm font-medium sm:col-span-2">Текущий пароль<input name="currentPassword" type="password" autoComplete="current-password" required className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--white)] px-3 text-sm font-normal outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block text-sm font-medium">Новый пароль<input name="newPassword" type="password" autoComplete="new-password" minLength={8} required className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--white)] px-3 text-sm font-normal outline-none focus:border-[color:var(--accent)]" /></label>
          <label className="block text-sm font-medium">Подтвердите новый пароль<input name="passwordConfirmation" type="password" autoComplete="new-password" minLength={8} required className="mt-2 min-h-11 w-full border border-[color:var(--border)] bg-[color:var(--white)] px-3 text-sm font-normal outline-none focus:border-[color:var(--accent)]" /></label>
      ...
      <nav className="account-dashboard-links mt-10 grid gap-3 sm:grid-cols-3">
        <Link href="/account/orders" className="border border-[color:var(--border)] px-5 py-4 text-sm font-medium hover:border-[color:var(--accent)] hover:text-[color:var(--accent)]">Мои заказы</Link>
