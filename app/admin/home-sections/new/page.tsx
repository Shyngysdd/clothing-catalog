...
export default async function NewHomeSectionPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [products, categories] = await Promise.all([prisma.product.findMany({ select: { id: true, name: true, price: true }, orderBy: { name: "asc" } }), prisma.category.findMany({ select: { slug: true, nameRu: true }, orderBy: { nameRu: "asc" } })]);
