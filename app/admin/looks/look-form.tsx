...
export function LookForm({ look, products, action }: { look?: EditableLook; products: ProductOption[]; action: (formData: FormData) => void | Promise<void> }) {
  ...
  return <form onSubmit={submit} className="mt-8 space-y-8">
    ...
    <section><h2 className="font-display text-3xl leading-none">Кадры образа</h2><div className="mt-4 flex flex-wrap gap-2">{toneOptions.map((tone) => <label key={tone} className="cursor-pointer"><input type="checkbox" name="photoTones" value={tone} checked={tones.includes(tone)} onChange={() => toggleTone(tone)} className="peer sr-only" /><span className="inline-flex min-h-10 items-center border border-[color:var(--ink)]/25 px-3 text-sm peer-checked:border-[color:var(--ink)] peer-checked:bg-[color:var(--ink)] peer-checked:text-[color:var(--white)]">{tone}</span></label>)}</div><label className="mt-5 block text-sm font-medium">Фото образа<input name="lookPhotoFile" type="file" multiple accept="image/jpeg,image/png,image/webp" className="mt-2 block min-h-11 w-full cursor-pointer border border-[color:var(--ink)]/25 px-3 py-2 text-sm" /><span className="mt-2 block text-xs font-normal text-[color:var(--ink)]/55">Можно выбрать несколько фото. Они будут показаны в галерее вместо кадров-заглушек.</span></label>{look?.photoUrls.length ? <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">{look.photoUrls.map((photoUrl, index) => <label key={photoUrl} className="border border-[color:var(--ink)]/15 p-2"><span className="relative block aspect-[3/4] overflow-hidden"><Image src={photoUrl} alt={`Фото образа ${index + 1}`} fill sizes="160px" className="object-cover" /></span><span className="mt-2 flex items-center gap-2 text-xs"><input type="checkbox" name="removeLookPhoto" value={photoUrl} className="accent-[color:var(--accent)]" />Удалить</span></label>)}</div> : null}{uploadError ? <p className="mt-3 text-sm text-[color:var(--accent)]">{uploadError}</p> : null}</section>
    <section><div className="flex items-end justify-between gap-4"><h2 className="font-display text-3xl leading-none">Состав</h2><p className="font-mono-price text-lg">{formatPrice.format(totalPrice)} ₸</p></div><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Поиск по названию" className="mt-5 min-h-11 w-full border border-[color:var(--ink)]/25 px-3 text-sm outline-none focus:border-[color:var(--ink)]" /><div className="mt-3 max-h-80 divide-y divide-[color:var(--ink)]/10 overflow-y-auto border border-[color:var(--ink)]/15">{visibleProducts.map((product) => <label key={product.id} className="flex cursor-pointer items-center gap-3 px-3 py-3 hover:bg-[color:var(--paper)]"><input type="checkbox" name="productIds" value={product.id} checked={selectedIds.includes(product.id)} onChange={() => toggleProduct(product.id)} className="size-4 accent-[color:var(--accent)]" />{product.imageUrl ? <span className="relative size-8 shrink-0 overflow-hidden border border-[color:var(--ink)]/10"><Image src={product.imageUrl} alt="" fill sizes="32px" className="object-cover" /></span> : <span className="size-8 shrink-0 border border-[color:var(--ink)]/10" style={{ backgroundColor: product.imageColor }} />}<span className="min-w-0 flex-1 truncate text-sm">{product.name}</span><span className="font-mono-price text-xs">{formatPrice.format(product.price)} ₸</span></label>)}</div><p className="mt-3 text-sm text-[color:var(--ink)]/60">Выбрано товаров: {selectedProducts.length}</p></section>
