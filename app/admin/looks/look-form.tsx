...
type ProductOption = { id: string; nameRu: string; price: number; imageColor: string; imageUrl: string | null };
type EditableLook = { titleRu: string; titleEn: string | null; titleKz: string | null; descriptionRu: string; descriptionEn: string | null; descriptionKz: string | null; photoTones: string[]; photoUrls: string[]; items: { productId: string }[] };
...
export function LookForm({ look, products, action }: { look?: EditableLook; products: ProductOption[]; action: (formData: FormData) => void | Promise<void> }) {
  ...
  const visibleProducts = useMemo(() => products.filter((product) => product.nameRu.toLowerCase().includes(search.toLowerCase())), [products, search]);
  ...
  return <form onSubmit={submit} className="mt-8 space-y-8">
    <section>
      <h2 className="font-display text-3xl leading-none">Тексты образа</h2>
      <div className="mt-4"><AdminLocaleTabs panels={{
        ru: <div className="grid gap-5"><label className="block">Название <span className="font-mono-price text-[10px] text-[color:var(--accent)]">ОБЯЗАТЕЛЬНО</span><input name="titleRu" required defaultValue={look?.titleRu} className="mt-1 min-h-11 w-full border border-[color:var(--border)] px-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label><label className="block">Описание <span className="font-mono-price text-[10px] text-[color:var(--accent)]">ОБЯЗАТЕЛЬНО</span><textarea name="descriptionRu" required defaultValue={look?.descriptionRu ?? ""} className="mt-1 min-h-28 w-full border border-[color:var(--border)] px-3 py-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label></div>,
        en: <div className="grid gap-5"><label className="block">Название<input name="titleEn" defaultValue={look?.titleEn ?? ""} placeholder={OPTIONAL_TRANSLATION_PLACEHOLDER} className="mt-1 min-h-11 w-full border border-[color:var(--border)] px-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label><label className="block">Описание<textarea name="descriptionEn" defaultValue={look?.descriptionEn ?? ""} placeholder={OPTIONAL_TRANSLATION_PLACEHOLDER} className="mt-1 min-h-28 w-full border border-[color:var(--border)] px-3 py-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label></div>,
        kz: <div className="grid gap-5"><label className="block">Название<input name="titleKz" defaultValue={look?.titleKz ?? ""} placeholder={OPTIONAL_TRANSLATION_PLACEHOLDER} className="mt-1 min-h-11 w-full border border-[color:var(--border)] px-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label><label className="block">Описание<textarea name="descriptionKz" defaultValue={look?.descriptionKz ?? ""} placeholder={OPTIONAL_TRANSLATION_PLACEHOLDER} className="mt-1 min-h-28 w-full border border-[color:var(--border)] px-3 py-3 text-sm outline-none focus:border-[color:var(--ink)]" /></label></div>,
      }} /></div>
    </section>
