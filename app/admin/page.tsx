...
export default async function AdminPage() {
  ...
  return <main className="mx-auto max-w-[90rem] px-4 py-10 sm:px-6 sm:py-16 lg:px-10">
    ...
    <section aria-label="Ключевые показатели" className="mt-8 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{metrics.map((metric) => <article key={metric.label} className="border border-[color:var(--ink)]/15 bg-[color:var(--white)] p-5 transition-shadow hover:shadow-[4px_4px_0_var(--gold)]"><p className="font-display text-5xl leading-none tracking-[-0.05em]">{metric.value}</p><p className="mt-4 font-mono-price text-[10px] tracking-[0.11em] text-[color:var(--ink)]/60">{metric.label}</p></article>)}</section>
