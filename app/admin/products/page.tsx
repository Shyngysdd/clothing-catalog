...
export default async function AdminProductsPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  const products = await prisma.product.findMany({ where: query ? { OR: [{ nameRu: { contains: query, mode: "insensitive" } }, { sku: { contains: query, mode: "insensitive" } }] } : undefined, include: { sizes: true, category: true }, orderBy: { createdAt: "desc" } });
